        ...
        <Stack justifyContent={'flex-start'} mt={is480 ? 3 : 5} mb={'14rem'} alignItems={'center'}>
            {
                wishlistFetchStatus === 'pending' ?
                    ...
                    <Stack width={is1130 ? "auto" : '70rem'} rowGap={is480 ? 2 : 4}>
                        ...
                        <Stack alignSelf={'flex-start'} flexDirection={'row'} columnGap={1} justifyContent={'center'}
                               alignItems={'center'}>
                            <motion.div whileHover={{x: -5}}>
                                <IconButton component={Link} to={'/'}><ArrowBackIcon
                                    fontSize={is480 ? 'medium' : 'large'}/></IconButton>
                            </motion.div>
                        ...
                        <Stack>

                            {
                                !wishlistFetchStatus === 'pending' && wishlistItems?.length === 0 ? (
                                        ...
                                        <Stack minHeight={'60vh'} width={is642 ? 'auto' : '40rem'} justifySelf={'center'}
                                               alignSelf={'center'} justifyContent={'center'} alignItems={'center'}>
                                            <Lottie animationData={emptyWishlistAnimation}/>
                                            <Typography variant='h6' fontWeight={300}>You have no items in your
                                                wishlist</Typography>
                                        </Stack>
                                    ) :
                                    // wishlist grid
                                    <Grid container gap={1} justifyContent={'center'} alignContent={'center'}>
                                        {
                                            wishlistItems.map((item, index) => (
                                                <Stack component={is480 ? "" : Paper} elevation={1}>
                                                    ...
                                                    <ProductCard item key={item._id} brand={item.product.brand.name}
                                                                 id={item.product._id} price={item.product.price}
                                                    ...
                                                    <Stack paddingLeft={2} paddingRight={2} paddingBottom={2}>
                                                        ...
                                                        <Stack flexDirection={'row'} alignItems={'center'}>
                                                            ...
                                                            <IconButton
                                                                onClick={() => handleEdit(index)}><EditOutlinedIcon/></IconButton>
                                                        </Stack>

                                                        {
                                                            editIndex === index ? (
                                                                    ...
                                                                    <Stack rowGap={2}>
                                                                        ...
                                                                        <TextField multiline rows={4} value={editValue}
                                                                ...
                                                                <Box>
                                                                    ...
                                                                    }}>{item.note ? item.note : "Add a custom note here"}</Typography>
                                                                </Box>
                                                        }

                                                        {
                                                            cartItems.some((cartItem) => cartItem.product._id === item.product._id) ?
                                                                <Button sx={{mt: 4}} size='small' variant='outlined'
                                                                        component={Link} to={'/cart'}>Already in
                                                                    cart</Button> : <Button sx={{mt: 4}} size='small'
                                                                                            onClick={() => handleAddToCart(item.product._id)}
                                                                                            variant='outlined'>Add To
                                                                    Cart</Button>
                                                        }
