...
export const Address = ({id, type, street, postalCode, country, phoneNumber, state, city}) => {
    ...
    return (
        <Stack width={'100%'} p={is480 ? 0 : 1}>
            ...
            {/* address details */}
            <Stack p={2} position={'relative'} flexDirection={'column'} rowGap={1} component={'form'} noValidate
                ...
                {/* action buttons */}
                <Stack position={is480 ? "static" : edit ? "static" : 'absolute'} bottom={4} right={4}
                       mt={is480 ? 2 : 4} flexDirection={'row'} alignSelf={'flex-end'} columnGap={1}>

                    {/* if edit is true, then save changes button is shown instead of edit*/}
                    {
                        edit ? (<LoadingButton loading={status === 'pending'} size='small' type='submit'
                    ...
                    {/* if edit is true then cancel button is shown instead of remove */}
                    {
                        edit ? (
                            ...
                            <LoadingButton loading={status === 'pending'} size='small' onClick={handleRemoveAddress}
                                           variant='outlined' color='error'>Remove</LoadingButton>
                        )
                    }
                </Stack>
            </Stack>
