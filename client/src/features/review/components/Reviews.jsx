...
export const Reviews = ({productId, averageRating}) => {
    ...
    return (
        <Stack rowGap={5} alignSelf={"flex-start"} width={is480 ? "90vw" : is840 ? "25rem" : '40rem'}>
                    ...
                    reviews?.length ? (
                        <Stack rowGap={3}>
                            ...
                            <Stack rowGap={1}>
                                ...
                                <Typography variant='h6' color={'text.secondary'}>Based
                                    on {reviews.length} {reviews.length === 1 ? "Review" : "Reviews"}</Typography>
                            </Stack>
                            ...
                            <Stack rowGap={2}>
                                {
                                    [5, 4, 3, 2, 1].map((number) => (
                                        <Stack flexDirection={'row'} justifyContent={'space-between'}
                                            ...
                                            <LinearProgress sx={{width: "100%", height: "1rem", borderRadius: "4px"}}
                                                            variant='determinate'
                        ...
                        <Typography variant='h6' color={'text.secondary'}
                                    fontWeight={400}>{loggedInUser?.isAdmin ? "There are no reviews currently" : "Be the one to post review first"}</Typography>
                    )
            ...
            <Stack rowGap={2}>
                {reviews?.map((review) => (
                    <ReviewItem key={review._id} id={review._id} userid={review.user._id} comment={review.comment}
                                createdAt={review.createdAt} rating={review.rating} username={review.user.name}/>))}
            </Stack>

            {
                ...
                writeReview ?
                    (
                        <Stack rowGap={3} position={'relative'} component={'form'} noValidate
                               onSubmit={handleSubmit(handleAddReview)}>

                            <TextField id='reviewTextFeild' {...register("comment", {required: true})}
                                       sx={{mt: 4, width: is840 ? "100%" : "40rem"}} multiline rows={6} fullWidth
                                       placeholder='Write a review...'/>

                            <Stack>
                                <Typography gutterBottom variant='body2'>How much did you like the product?</Typography>
                                <motion.div style={{width: "fit-content"}} whileHover={{scale: 1.050, x: 2}}
                                            whileTap={{scale: 1}}>
                                    <Rating size='large' value={value} onChange={(e) => setValue(e.target.value)}/>
                                </motion.div>
                            </Stack>

                            <Stack flexDirection={'row'} alignSelf={'flex-end'} alignItems={'center'}
                                   columnGap={'.2rem'}>
                                <MotionConfig whileTap={{scale: 1}} whileHover={{scale: 1.050}}>
                                    <motion.div>
                                        <LoadingButton sx={{textTransform: "none", fontSize: is480 ? "" : "1rem"}}
                                                       size={is480 ? "small" : ""} loading={reviewStatus === 'pending'}
                                                       type='submit' variant='contained'>Add review</LoadingButton>
                                    </motion.div>
                                    <motion.div>
                                        <Button onClick={() => setWriteReview(false)} color='error'
                                                size={is480 ? "small" : ""} variant='outlined' sx={{
                                            textTransform: "none",
                                            fontSize: is480 ? "" : "1rem"
                                        }}>Cancel</Button>
                                    </motion.div>
                    ...
                    !loggedInUser?.isAdmin ?
                        <motion.div onClick={() => setWriteReview(!writeReview)} whileHover={{scale: 1.050}}
                                    whileTap={{scale: 1}} style={{width: "fit-content"}}>
                            <Button disableElevation size={is480 ? "medium" : 'large'} variant='contained' sx={{
                                color: theme.palette.primary.light,
                                textTransform: "none",
                                fontSize: "1rem",
                                borderRadius: '6px'
                            }} startIcon={<CreateIcon/>}>Write a review</Button>
                        </motion.div> : ""
            }
        </Stack>
    )
