            ...
            <Stack flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'}>
                ...
                <Stack flexDirection={'row'} columnGap={2}>
                    <Stack>
                        ...
                        <motiondiv>
                            <Rating size={edit ? is480 ? 'medium' : 'large' : "small"} readOnly={!edit}
                                    onChange={(e) => setEditRating(e.target.value)} value={edit ? editRating : rating}/>
                        </motiondiv>
                    </Stack>
                </Stack>

                {
                    isOwnReview && (
                        <Stack sx={{position: 'absolute', top: 0, right: 0}}>
                            ...
                            <IconButton aria-controls={open ? 'basic-menu' : undefined} aria-haspopup="true"
                ...
                <Typography alignSelf={"flex-end"} justifySelf={'flex-end'} color={'text.secondary'} fontWeight={400}
                            fontSize={'.9rem'}>{new Date(createdAt).toDateString()}</Typography>
            </Stack>
            ...
            <Stack>
                {
                    edit ? (
                        <Stack component={'form'} noValidate onSubmit={handleSubmit(handleUpdateReview)} rowGap={2}>
                            ...
                            <Stack flexDirection={'row'} alignSelf={'flex-end'} columnGap={1} justifyContent={'center'}
                                   alignItems={'center'}>
                                <LoadingButton size='small' type='submit' sx={{alignSelf: "flex-end"}}
